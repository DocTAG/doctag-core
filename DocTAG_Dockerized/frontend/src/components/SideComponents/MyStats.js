...
//             //     }
//
//             axios.get("http://127.0.0.1:8000/get_stats_array_per_usecase",{params:{mode:ChosenStats}}).then(response => {
...
function MyStats() {
    ...
    useEffect(()=>{
        if(ChosenStats !== '' && SelectedLang !== '' && SelectedInstitute !== '' && SelectedBatch !== ''){
            ...
            axios.get("http://127.0.0.1:8000/get_stats_array_per_usecase",{params:{mode:ChosenStats,language:SelectedLang,institute:SelectedInstitute,batch:SelectedBatch}}).then(response => {
                SetStatsArrayPercent(response.data['medtag']['percent'])
                SetStatsArray(response.data['medtag']['original'])
                SetStatsArrayPercentPubMed(response.data['pubmed']['percent'])
                SetStatsArrayPubMed(response.data['pubmed']['original'])
        ...
        <div className="App">
            <div>
                <Container fluid>
                    ...
                    {(ChosenStats !== '' ) && SelectedLang !== '' && <div style={{'text-align':'center'}}>
                        {(StatsArray && StatsArrayPercent && StatsArrayPubMed && StatsArrayPercentPubMed) ? <div>
