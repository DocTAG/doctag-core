...
def new_credentials(request):
    ...
    if request.method == 'POST':
        ...
        # cursor.execute("SELECT DISTINCT institute FROM report AS r INNER JOIN topic_has_document AS t ON t.id_report = r.id_report AND r.language = t.language WHERE t.name = %s",[str(usecase)])
        # ans = cursor.fetchall()
        # if ans[0] == 'default':
        #     request.session['report_type'] = 'reports'
        # elif ans[0] == 'PUBMED':
        #     request.session['report_type'] = 'pubmed'
        #     request.session['institute'] = 'PUBMED'
...
def annotationlabel(request, action=None):
    ...
    if request.method == 'POST' and action.lower() == 'insert':
        ...
        if len(labels_to_save) == 0 and mode1 == 'Human':
            ...
            rows = Associate.objects.filter(name=topic_obj, username=user, ns_id=mode, id_report=report1,
                                            language=language)
            if rows.exists():
                try:
                    with transaction.atomic():
        ...
        if existing_rows.exists():
            if existing_rows.count() == len(labels_to_save):
                for label in labels_to_save:
                    ...
                    if not Associate.objects.filter(username=user, ns_id=mode, seq_number=label1.seq_number,
                                                    label=label1,
        ...
        if update == True:
            try:
                with transaction.atomic():
                    ...
                    json_resp_labels = update_annotation_labels(labels_to_save, user, report1, language, mode,
                                                                topic_obj)
...
def mention_insertion(request, action=None):
    ...
    if request.method == 'GET':
        ...
        ass = Annotate.objects.filter(username=user, ns_id=mode, id_report=report1.id_report, language=language,
                                      name=topic_obj).values('start', 'stop', 'label')
        if len(ass) == 0:
        ...
        if len(mentions) == 0 and mode1 == 'Human':
            ...
            if Annotate.objects.filter(name=topic, username=user, ns_id=mode, id_report=report1,
        ...
        if existing_rows.exists():
            if existing_rows.count() == len(mentions):
                ...
                for ment in mentions:
                    ...
                    mentionDB = Mention.objects.filter(start=int(ment['start']), stop=int(ment['stop']),
                    ...
                    if mentionDB.exists():
                        ...
                        anno = Annotate.objects.filter(username=user, ns_id=mode, name=topic_obj, id_report=report1,
                                                       language=language, label=annoDB,
        ...
        if update == True:
            try:
                with transaction.atomic():
                    ...
                    if GroundTruthLogFile.objects.filter(username=user, ns_id=mode, language=language,
                                                         id_report=report1, name=topic_obj,
                                                         gt_type='mentions').exists():
                        GroundTruthLogFile.objects.filter(username=user, ns_id=mode, language=language,
                                                          id_report=report1, name=topic_obj,
                                                          gt_type='mentions').delete()
                    jsonDict = serialize_gt(type, username, report_id, language, mode, topic)
                    GroundTruthLogFile.objects.create(username=user, ns_id=mode, id_report=report1, language=language,
                                                      name=topic_obj, gt_json=jsonDict, gt_type=type,
                                                      insertion_time=Now())
...
def insert_link(request, action=None):
    ...
    if request.method == 'GET' and action.lower() == 'mentions':
        ...
        report1 = Report.objects.get(id_report=report_id, language=language)
        try:
            a = Annotate.objects.filter(name=topic_obj.name, username=user_get, ns_id=mode_1, id_report=report1,
                                        language=language).values('start', 'stop', 'label', 'seq_number')
            b = Linked.objects.filter(topic_name=topic_obj.name, username=user_get, ns_id=mode_1, id_report=report1,
                                      language=language).values('start', 'stop')
            json_dict = {}
            json_dict['mentions1'] = []
            for el in a:
        ...
        if len(mentions) > 0:
            try:
                with transaction.atomic():
                    mention = mentions[0]
                    m = Mention.objects.filter(start=mention['start'], stop=mention['stop'], id_report=report1,
                                               language=language)
                    if not Mention.objects.filter(start=mention['start'], stop=mention['stop'], id_report=report1,
                                                  language=language).exists():
                        Mention.objects.create(start=mention['start'], stop=mention['stop'],
                                               mention_text=mention['mention_text'], id_report=report1,
                                               language=language)
        ...
        if len(concepts) == 0 and mode1 == 'Human':
            if Linked.objects.filter(username=user, ns_id=mode, id_report=report1, language=language,
                                     topic_name=topic_obj.name).exists():
                try:
                    with transaction.atomic():
                        json_response = delete_all_associations(user, report1, language, type, mode, topic_obj)
                    return JsonResponse(json_response)
        ...
        if update == True:
            try:
                with transaction.atomic():
                    ...
                    if GroundTruthLogFile.objects.filter(username=user, ns_id=mode, name=topic_obj, language=language,
                                                         id_report=report1, gt_type='concept-mention').exists():
                        obj = GroundTruthLogFile.objects.filter(name=topic_obj, username=user, ns_id=mode,
                                                                language=language, id_report=report1,
                                                                gt_type='concept-mention')
                        obj.delete()
                    jsonDict = serialize_gt(type, username, report_id, language, mode, topic)
                    GroundTruthLogFile.objects.create(username=user, name=topic_obj, ns_id=mode, language=language,
                                                      id_report=report1, gt_json=jsonDict, gt_type=type,
                                                      insertion_time=Now())
                    return JsonResponse(json_response)
            ...
            try:
                with transaction.atomic():
                    if mode1 == 'Human':
                        ...
                        user_robot = User.objects.get(username='Robot_user', ns_id=mode)
                        gt_robot = GroundTruthLogFile.objects.filter(username=user_robot, name=topic_obj, ns_id=mode,
                                                                     id_report=report1, language=language,
                                                                     gt_type='concept-mention')
                        # in questa sezione solo se la gt è uguale a prima, l'utente acconsente alla gt della macchina
                        gt = GroundTruthLogFile.objects.filter(username=user, ns_id=mode, id_report=report1,
                        ...
                        if gt.count() == 1 and gt_robot.count() == 1:
                            if gt_robot[0].insertion_time == gt[0].insertion_time:
                                ...
                                GroundTruthLogFile.objects.create(gt_json=js, insertion_time=Now(), name=topic_obj,
                                                                  username=user, ns_id=mode, id_report=report1,
                                                                  language=language, gt_type='concept-mention')
                                ass = Linked.objects.filter(username=user, id_report=report1, language=language,
                                                            topic_name=topic_obj, ns_id=mode).values('start', 'stop',
                                                                                                     'name',
                                                                                                     'concept_url')
                                for el in ass:
                                    men_cur = Mention.objects.get(id_report=report1, language=language,
                                    ...
                                    Linked.objects.filter(topic_name=topic_obj, username=user, id_report=report1,
                                                          language=language, ns_id=mode, name=sem, start=men_cur,
                                                          stop=el['stop'], concept_url=concept_u).delete()
                                    Linked.objects.create(topic_name=topic_obj, username=user, ns_id=mode,
                                                          id_report=report1, language=language, name=sem,
                                                          stop=el['stop'], start=men_cur, concept_url=concept_u,
                                                          insertion_time=Now())
        ...
        if request.method == 'GET':
            ...
            if report is not None and concepts_list is not None:
                ...
                try:
                    with transaction.atomic():
                        for concept in concepts_list:
                            ...
                            if not check_concept_report_existance(report, concept_url, user, mode, semantic_area,
                                                                  language, topic_obj):
                                # Insert a new record
                ...
                if count == len(concepts_list):
                    ...
                    response_json = {"message": "Some concepts have been already inserted: [" + ", ".join(
                        already_inserted_list) + "]"}
            else:
            ...
            if report is not None and concepts_list is not None:
                user = username
                count = 0
                rows = Contains.objects.filter(topic_name=topic_obj.name, username=user1, ns_id=mode, id_report=report1,
                                               language=language)
                ...
                if rows.exists():
                    if rows.count() == len(concepts_list):
                        for concept in concepts_list:
                            ...
                            concepts = Contains.objects.filter(topic_name=topic_obj.name, name=semantic_area,
                                                               username=user1, ns_id=mode, id_report=report1,
                                                               language=language, concept_url=concept_model)
                            if concepts.exists():
                                update = False
                            else:
                                update = True
                ...
                if update == True:
                    try:
                        with transaction.atomic():
                            ...
                            Contains.objects.filter(username=user1, ns_id=mode, id_report=report1, language=language,
                                                    topic_name=topic).delete()
                            ...
                            for concept in concepts_list:
                                ...
                                if populate_contains_table(report, concept_url, user, mode, semantic_area, language,
                                    ...
                                    return JsonResponse(error_json)
                            jsonDict = serialize_gt(type, username, report, language, mode, topic)
                            if GroundTruthLogFile.objects.filter(username=user1, ns_id=mode, id_report=report1,
                                                                 language=language, gt_type=type,
                                                                 name=topic_obj).exists():
                                GroundTruthLogFile.objects.filter(username=user1, ns_id=mode, id_report=report1,
                                                                  language=language, gt_type=type,
                                                                  name=topic_obj).delete()
                                    ...
                                    if gt_robot[0].insertion_time == gt[0].insertion_time:
                                        ...
                                        for el in ass:
                                            ...
                                            Contains.objects.filter(username=user1, id_report=report1,
                                                                    language=language, topic_name=topic,
                                                                    ns_id=mode, name=sem,
...
def get_reports(request):
    ...
    if (inst != None and use != None and lang != None and batch != None):
        ...
        cursor = connection.cursor()
        cursor.execute(
            "SELECT r.id_report FROM report AS r INNER JOIN topic_has_document AS t ON t.id_report = r.id_report AND r.language = t.language WHERE r.institute=%s AND r.language = %s AND r.batch = %s AND t.name = %s",
...
def check_input_files(request):
    ...
    jsonResp = check_file(reports, pubmedfiles, labels, concepts, jsonDisp, jsonAnn, username, password, topic, runs,
                          tfidf)
...
def get_gt_list(request):
    ...
    if token == 'all':
        ...
        list_gt = GroundTruthLogFile.objects.filter(username=rob_user).count() + GroundTruthLogFile.objects.filter(
            ns_id=ns_human).count()
        ...
        # print(groundTruths)
        for el in gt_rob:
            gts = GroundTruthLogFile.objects.filter(ns_id=ns_robot, gt_type=el.gt_type, id_report=el.id_report_id,
                                                    language=el.language).exclude(insertion_time=el.insertion_time)
            gts_count = gts.count()
...
def check_files_for_update(request):
    ...
    # load_concepts = request.POST.get('exa_concepts',None)
    # load_labels = request.POST.get('exa_labels',None)
    msg = check_for_update(type1, pubmedfiles, reports, labels, concepts, jsonDisp, jsonAnn, jsonDispUp, jsonAnnUp,
                           topic, runs, tf_idf)
...
def update_db(request):
    ...
    # print(batch)
    msg = update_db_util(reports, pubmedfiles, labels, concepts, jsonDisp, jsonAnn, jsonDispUp, jsonAnnUp, jsonAll,
                         topic, runs, batch, tf_idf)
...
def configure_db(request):
    ...
    username = request.POST.get('username', None)
    password = request.POST.get('password', None)
    msg = configure_data(pubmedfiles, reports, labels, concepts, jsonDisp, jsonAnn, jsonAll, username, password, topic,
                         runs, tf_idf)
...
def get_reports_from_action(request):
    ...
    cursor = connection.cursor()
    cursor.execute(
        "SELECT r.id_report,r.insertion_time FROM ground_truth_log_file AS r INNER JOIN topic_has_document AS t ON t.id_report = r.id_report AND t.language = r.language WHERE r.language = %s AND t.name = %s AND r.ns_id = %s AND r.gt_type = %s AND r.username = %s ORDER BY insertion_time DESC",
        [language, topic, mode1, action, username])
    ans = cursor.fetchall()
...
def conc_view(request):
    ...
    if mode == 'Human' or auto_required == 'Human':
        cursor = connection.cursor()
        cursor.execute(
            "SELECT DISTINCT b.name FROM belong_to as b inner join concept as c on c.concept_url = b.concept_url")
        ar = cursor.fetchall()
        areas = []
        for el in ar:
            areas.append(el[0])
    ...
    if report is not None or all == 'all':
        ...
        if all == 'all':
            ...
            if Report.objects.filter(institute='PUBMED').exists():
                json_resp['all_fields'].extend(
                    ['title', 'abstract', 'volume', 'journal', 'year', 'authors'])  # aggiungo pubmed solo in coda!
    else:
        if request.session['report_type'] == 'pubmed':
            ...
            json_resp = get_fields_from_json_configuration(request.session['usecase'], request.session['institute'],
...
def get_data(request):
    ...
    for el in reports:
        ...
        for top in topic:
            ...
            cursor = connection.cursor()
            cursor.execute(
                "SELECT * FROM ground_truth_log_file AS g INNER JOIN topic_has_document AS t ON t.id_report = g.id_report AND g.language = t.language WHERE t.id_report = %s AND t.language = %s AND t.name = %s",
                [str(el.id_report), str(el.language), str(top['name'])])
            ...
            json_resp['reports'].append(
                {'total': total, 'report': new_rep, 'id_report': el.id_report, 'language': el.language})
...
def doctag_reports(request):
    json_resp = {}
    json_resp['usecase'] = []
    reps = TopicHasDocument.objects.all()
    for r in reps:
        report = r.id_report_id
        print(report)
        if not report.startswith('PUBMED_') and not str(r.name_id) in json_resp['usecase']:
...
def pubmed_reports(request):
    ...
    for r in reps:
        ...
        if report.startswith('PUBMED_') and not str(r.name_id) in json_resp['usecase']:
            json_resp['usecase'].append(str(r.name_id))
    return JsonResponse(json_resp)
        ...
        with transaction.atomic():
            ...
            for report in report_list:
                ...
                if rp.count() == 1:
                    ...
                    rp.delete()
                    if TopicHasDocument.objects.filter(id_report=report['id_report'],
                                                       language=report['language']).count() == 0:
...
def get_insertion_time_record(request):
    ...
    gt_user = GroundTruthLogFile.objects.filter(name=topic, id_report=report1, language=language, ns_id=ns_id,
                                                username=user, gt_type=action)
    if gt_user.exists():
        gt_user = GroundTruthLogFile.objects.get(name=topic, id_report=report1, language=language, ns_id=ns_id,
                                                 username=user, gt_type=action)
        ...
        if user_obj == request.session['username'] and ns_id_str == 'Robot':
            ...
            gt_rob = GroundTruthLogFile.objects.get(name=topic, id_report=report1, language=language, ns_id=ns_id,
                                                    username=user_rob, gt_type=action)
