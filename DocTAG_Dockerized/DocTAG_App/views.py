...
def new_credentials(request):
    ...
    if request.method == 'POST':
        ...
        ns_id = NameSpace.objects.get(ns_id = 'Human')
        batch = request_body_json['batch']
        request.session['batch'] = batch

        if usecase == '':
            gt = get_last_groundtruth(username=request.session['username'], mode=ns_id, language=language,
                                      institute=institute,batch = batch)
            if gt is not None:
                gt = json.loads(gt[0])
                request.session['usecase'] = gt['topic_id']
            else:
                cursor = connection.cursor()
                cursor.execute("SELECT DISTINCT t.name FROM report as r inner join topic_has_document as t on t.id_report = r.id_report and r.language = t.language WHERE r.institute = %s and t.language = %s and r.batch = %s ",[institute,language,batch])
                ans = cursor.fetchall()
                request.session['usecase'] = ans[0][0]
    ...
    try:
        with transaction.atomic():
            ...
            for report in report_list:
                ...
                rp = TopicHasDocument.objects.filter(id_report = report['id_report'],language = report['language'],name = use)
                if rp.count() == 1:
                    rp = rp.first()
                    ...
                    Associate.objects.filter(id_report = rep,language=rp.language,name = use).delete()
                    Contains.objects.filter(id_report = rep,language=rp.language,topic_name = use).delete()
                    GroundTruthLogFile.objects.filter(id_report = rep,language=rp.language,name = use).delete()
                    rp.delete()
...
#         batch = Report.objects.filter(name=use_obj).exclude(institute = 'PUBMED').values('batch')
#     else:
#         batch = Report.objects.all().values('batch')
#     for el in batch:
...
def get_annotators_users_list(request):
    ...
    for name in users:
        ...
        if u.count() == 1 and name['username'] != 'Robot_user':
            cursor = connection.cursor()
            cursor.execute("SELECT * FROM ground_truth_log_file AS g INNER JOIN report AS r ON r.id_report = g.id_report and g.language = r.language INNER JOIN topic_has_document AS t ON t.id_report = r.id_report and r.language = r.language WHERE r.institute = %s AND r.language = %s AND t.name = %s AND ns_id = %s AND username = %s AND r.id_report = COALESCE(%s, r.id_report) AND g.gt_type = %s",
